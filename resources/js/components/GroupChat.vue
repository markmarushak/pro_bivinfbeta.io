            ...
            <div class="card">
                ...
                <div class="card-body msg_card_body">
                    <!--<div class="d-flex justify-content-start mb-4">-->
                        ...
                        <!--</div>-->
                    <!--</div>-->
                    <div class="d-flex justify-content-end mb-4" v-for="message in conversations">
                ...
                <div class="card-footer">
                    <div class="input-group">
                        ...
                        <div class="input-group-append">
                            <span class="input-group-text send_btn" @click="store" ><i class="fas fa-location-arrow"></i></span>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    export default {
        ...
        mounted() {
            if(localStorage.key_chat != '')
            {
                this.group_id = localStorage.key_chat
            }
        ...
        methods: {
            store() {
                if(!!(this.group_id) == false){
                    this.group_id = Date.now().toString(36)+"-"+Math.ceil(Math.random()* 99999)
                    localStorage.key_chat = this.group_id
                }

                axios.post('/conversations', {message: this.message, group_id: this.group_id})
                .then((response) => {
                    this.conversations.push(this.message);

                    this.message = '';
                    this.$emit('NewMessage','12313')
